...
def pool_optimizer(
    ...
    # Set up points to optimize in
    # top_points = [X_train[i].ravel().copy() for i in chosen_top_point_indices]
    all_points = [x_test.ravel().copy() for x_test in X_pool]
    ...
    batch_size = 1024
    if use_tqdm:
        for i in trange(0, len(all_points), batch_size, desc="Finding max from pool", leave=False):
            res = objective1d(np.array(all_points[i:i+batch_size]))[0]
            opt_results.extend(res)
    else:
    ...
    # for x in all_points:
    #     res = objective1d(x)[0][0]
    #     opt_results.append(res)

    if logger is not None:
        logger.info(f"t={time.time()-start_time:.1f}s")

    # Potentially directly return optimization results
    if return_res:
        return opt_results

    indmax = np.argmin(opt_results)
    return np.array([indmax]), [opt_results[indmax]]


def gp_pool(gp_file, data_file, save_file, n_out, logfile):
    ...
    LOGGER.info(f"Number of training points={len(X_train)}")
    LOGGER.info(f"Number of pool points={len(X_pool)}")
    ...
    latent_pred, ei_vals = pool_optimizer(
    ...
    # Save results - it's just the index in the pool of the selected data
...
if __name__ == "__main__":
    ...
    gp_pool(args.gp_file, args.data_file, args.save_file, args.n_out, args.logfile)
